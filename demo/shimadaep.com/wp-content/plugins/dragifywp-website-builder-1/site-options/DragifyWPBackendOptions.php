...
class DragifyWPBackendOptions{
	...
	public function backend_enqueue_scripts()
	{
		if ($this->is_backend()) {
			...
			$localize_obj = array(
					...
					'save_options_nonce' => wp_create_nonce('dragifywp_save_options'),
					...
					'panels' => DragifyWPOptionsManager::fetch('backend'),
					'frontend_modules' => DragifyWPOptionsManager::getFrontendModules()
			);

			wp_register_script('dragifywp_backend_customizer', $this->_DIST_DIR . '/js/backend.min.js', array('angularjs', 'angular_sanitize', 'angular_ui_router'), DRAGIFYWP_APP_VERSION, true);
			wp_localize_script('dragifywp_backend_customizer', 'dragifywp_app_data', $localize_obj);
			wp_enqueue_script('dragifywp_backend_customizer');

			wp_enqueue_script('jquery-ui-slider');
			wp_enqueue_media();

			wp_register_script('select2', $this->_3RD.'/select2.full.min.js', array('jquery'), '4.0.5' ,false);
			...
			wp_register_style('dragifywp_backend_customizer', $this->_DIST_DIR . '/css/backend.min.css', array(), DRAGIFYWP_APP_VERSION ,'all');
			wp_enqueue_style('dragifywp_backend_customizer');

			wp_register_style('fontawesome', $this->_3RD.'/fontawesome/font-awesome.min.css',array(), '4.7.0', 'screen');
			wp_enqueue_style('fontawesome');

			wp_register_style('linearicon_min', $this->_3RD.'/linearicon-min/style.css', array(), DRAGIFYWP_APP_VERSION ,'all');
	...
	public function is_backend() {
		...
		if (array_intersect($allowed_roles, $user->roles ) && isset($_GET['page']) && $_GET['page'] == 'dragifywp-options') {
			return true;
		}
	...
	{
		?>
		<div class="wrap" ng-app="dragifywp.builder.app" ng-controller="dragifywp_builder_app_controller" ng-strict-di>
