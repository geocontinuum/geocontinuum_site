...
class DragifyWPLiveCustomizer{
	public function __construct() {
		$this->_DIST_DIR = DRAGIFYWP_SO_DIR.'/dist';
		$this->_IMG_DIR = DRAGIFYWP_SO_DIR.'/img';
	...
	public function frontend_enqueue_scripts()
	{
		...
		$redirect_links = array(
			array('text' => 'Preview This Page', 'link' => $frame_url, 'newTab'=>true, 'id' => 'dragifywp-preview-link')
		);

		$redirect_links[] = array('text' => 'Backend Editor', 'link' => '#', 'newTab'=> false, 'id' => 'dragifywp-backend-link');
		$redirect_links[] = array('text' => 'Header Builder', 'link' => '#', 'newTab' => false, 'id' => 'dragifywp-hb-link');
		$redirect_links[] = array('text' => 'Page Builder', 'link' => '#', 'newTab' => false, 'id' => 'dragifywp-pb-link');
		$redirect_links[] = array('text' => 'Footer Builder', 'link' => '#', 'newTab' => false, 'id' => 'dragifywp-fb-link');
		...
		$localize_data = array(
			'frame_url'  => add_query_arg(array('dragifywp-preview-enable' => 1), $frame_url),
			'is_front_end' => true,
			'root' => DRAGIFYWP_CORE_ROOT,
			'img_url' => $this->_IMG_DIR,
			'api' => get_rest_url() . 'dragifywp-site-options/v1',
			...
			'panels' => DragifyWPOptionsManager::fetch('frontend')
		);
		...
		wp_register_script('dragifywp_lc_frontend', $this->_DIST_DIR . '/js/frontend.min.js', array('angularjs'), DRAGIFYWP_APP_VERSION, true);
		wp_localize_script('dragifywp_lc_frontend', 'dragifywp_app_data', $localize_data);
		wp_enqueue_script('dragifywp_lc_frontend');		
		
		wp_enqueue_media();

		// Register & enqueue CSS
		wp_register_style('dragifywp_theme_option_frontend', $this->_DIST_DIR . '/css/frontend.min.css', array(), DRAGIFYWP_APP_VERSION ,'all');
		wp_enqueue_style('dragifywp_theme_option_frontend');
	...
	public function theme_options_area() {
		if($this->is_frontend()){
			...
			wp_head();
			echo '
			<body class="dragifywp-theme-option" ng-app="dragifywp.builder.app" ng-controller="dragifywp_builder_app_controller" ng-strict-di>
				<div class="dragifywp-app">
					<dragifywp-theme-option></dragifywp-theme-option>
	...
	public function preview_register_script() {
		if (!empty($_GET['dragifywp-preview-enable'])) {
	...
	public function preview_enqueue_script() {
		...
		if (is_singular()) {
			...
			if (class_exists('DragifyWPPageBuilder') && DragifyWPPageBuilder::post_type_is_accepted(get_post_type($id))) {
		...
		wp_register_script('site_options_iframe', $this->_DIST_DIR . '/js/iframe.min.js', array(), DRAGIFYWP_APP_VERSION ,false);
		wp_localize_script('site_options_iframe', 'dragifywp_st_data', $localize);
		wp_enqueue_script('site_options_iframe');
		wp_enqueue_style('site_options_iframe', $this->_DIST_DIR . '/css/iframe.css', array(), DRAGIFYWP_APP_VERSION ,'all');
		...
		if (!empty($options['uploaded_fonts'])) {
			...
			wp_add_inline_style('dragifywp.theme.option.iframe', $style);
		}
	...
	public function is_frontend() {
		...
		if (array_intersect($allowed_roles, $user->roles) && isset($_GET['page']) && $_GET['page'] == 'dragifywp-live-customizer') {
			return true;
		}
	...
	public function filter_body_classes($classes) {
		$classes[] = 'dragifywp-theme-option-iframe';
		return $classes;
	}
}

new DragifyWPLiveCustomizer();