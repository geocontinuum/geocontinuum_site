        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "default_value" => 14,
                    "styles" => array(
                        "@id .dragifywp-header__searchBox__input" => array(
                            "fontSize" => "{{@model}}px"
                        )
                    )
                ),
                array(
                    ...
                    "default_value" => 16,
                    "styles" => array(
                        "@id .dragifywp-header__searchBox__submit" => array(
                            "fontSize" => "{{@model}}px"
                        ),
                        "@id" => array(
                            "width" => "{{@model}}px"
                ...
                array(
                    ...
                    "default_value" => 16,
                    "styles" => array(
                        "@id.dragifywp-header__searchBox--active .dragifywp-header__searchBox__submit" => array(
                            "left" => "{{@model}}px"
                        )
                    )
                ),
        ...
        array(
            ...
            "content" => [
                array(
                    ...
                    "styles" => array(
                        "@id .dragifywp-header__searchBox__input,@id .dragifywp-header__searchBox__submit" => array(
                            "color" => "{{@model}}"
                        )
                    )
                ),
                array(
                    ...
                    "default_value" => $manager::$colors["heading"],
                    "styles" => array(
                        "@id.dragifywp-header__searchBox--active .dragifywp-header__searchBox__input,@id.dragifywp-header__searchBox--active .dragifywp-header__searchBox__submit" => array(
                            "color" => "{{@model}}"
                        )
                    )
                ),
                ...
                array(
                    ...
                    "default_value" => $manager::$colors["heading"],
                    "styles" => array(
                        "@id .dragifywp-header__searchBox__input:focus" => array(
                            "color" => "{{@model}}"
                        )
                    )
                ),
...
ob_start();
?>
<div class="dragifywp-header__searchBox {{::$ctrl.getID()}}"
     data-width="{{$ctrl.edited.attrs.width}}"
     ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]">
    <input class="dragifywp-header__searchBox__input" type="text" placeholder="{{$ctrl.edited.attrs.placeholder}}"/>
    <label class="dragifywp-header__searchBox__submit" dragifywp-toggle-search-box>
...
function lhb_h_search_box_callback($attrs) {
    ...
    $width = '';
    if(dragifywp_to_boolean($attrs['open'])) {
        $classes[] = 'dragifywp-header__searchBox--active';
    ...
    $html = '<div class="dragifywp-header__searchBox ' . implode(' ', $classes) . '" data-width="'.$attrs['width'].'"'.$width.'>';
    $html .= '<form role="search" method="get" action="' . get_home_url() . '">';
    $html .= '<input class="dragifywp-header__searchBox__input" type="text" name="s" placeholder="' . $attrs['placeholder'] . '" value="' . $s . '"'.$width.' />';
    $html .= '<label class="dragifywp-header__searchBox__submit"><i class="' . $icon . '"></i></label>';
    $html .= '</form>';
    $html .= '</div>';
