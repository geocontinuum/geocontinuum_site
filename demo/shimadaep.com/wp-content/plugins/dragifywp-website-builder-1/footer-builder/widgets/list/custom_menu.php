...
<div class="dragifywp-dnd__emptyItem" style="height:100px" ng-if="$ctrl.edited.content.length == 0">
    <span class="dragifywp-dnd__emptyItem__label">Custom Menu</span>
</div>

<ul class="dragifywp-menu {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.no_edge, $ctrl.edited.attrs.align,$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]">
    <li class="dragifywp-menu__item" ng-repeat="item in $ctrl.edited.content track by $index">
        <a model="item" dragifywp-editable config="{target:'attrs.name', editorType:'1'}" ng-bind-html="item.attrs.name"></a>
    </li>
</ul>
<?php
...
function lfb_custom_menu_callback($attrs, $content = null) {
    ...
    ob_start(); ?>
    <ul class="dragifywp-menu <?php echo implode(' ', $classes); ?>">
