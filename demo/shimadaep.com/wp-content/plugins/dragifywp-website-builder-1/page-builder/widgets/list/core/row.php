            ...
            "content" => [
                array(
                    ...
                    "responsive_styles" => array(
                        "@id > .dragifywp-row__container" => array(
...
ob_start();
?>
<div  class="dragifywp-row {{::$ctrl.getID()}}" ng-class="[$ctrl.row.attrs.class, $ctrl.row.attrs.hidden, $ctrl.row.attrs.flex, $ctrl.row.attrs.col_reverse, 'dragifywp-row--colAlign-'+$ctrl.row.attrs.column_align]" id="{{$ctrl.row.attrs.id}}">
    <div class="dragifywp-dnd__dropMe dragifywp-dnd__dropMe--lv2">
        <span></span>
        <span></span>
        <span></span>
        <span></span>
    ...
    <div class="dragifywp-dnd__visual dragifywp-dnd__visual--lv2 lvh__{{::$ctrl.getID()}}"
         dragifywp-detect-hover-node="$ctrl.row"
         ng-mousedown="$ctrl.service.hide = 'dragifywp-dnd__area--showList--lv2'">
        <div class="dragifywp-dnd__visual__label" dragifywp-keep-visual-label>
            <div class="dragifywp-dnd__visual__label__name" node-tree model='$ctrl.row'>
                <span ng-bind="::$ctrl.row.name" ng-click="$ctrl.service.setEdited($ctrl.row, true)"></span>
            </div>
        </div>
        <dragifywp-action current="$ctrl.row"></dragifywp-action>
    </div>

    <div class="dragifywp-row__container">
        <dragifywp-column-list row="$ctrl.row"></dragifywp-column-list>
    </div>
</div>
<?php
...
add_shortcode('dragifywp_row', 'dragifywp_row_callback');
function dragifywp_row_callback($attrs, $content = '') {
    ...
    if ($attrs['flex']){
        ...
        if($attrs['column_align']){
            $classes[] = 'dragifywp-row--colAlign-'.$attrs['column_align'];
        }
    }
    ...
    ob_start(); ?>
    <div class="dragifywp-row <?php echo implode(' ', $classes); ?>"<?php echo $id.$animation; ?>>
        <div class="dragifywp-row__container dragifywp-row__columnList"><?php echo do_shortcode($content); ?></div>
    </div>
    <?php
    return ob_get_clean();
}