...
class PD_Wp_Admin {
	...
	public function admin_bar_menu() {
		...
		if ( !empty( $_SERVER['REQUEST_URI'] ) ) {
			$desired_relative_path = $_SERVER['REQUEST_URI'];
			$relative_path_to_wp_directory = parse_url( site_url(), PHP_URL_PATH );
			if ( strpos( $desired_relative_path, $relative_path_to_wp_directory ) === 0 ) {
