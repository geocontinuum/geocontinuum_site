...
class DragifyWPHeaderBuilderAdmin
{
	...
	{
		$this->_DIST = DRAGIFYWP_HB_DIR.'/dist/';
		$this->_3RD = DRAGIFYWP_CORE_ROOT.'/modules/3rd/';
		$this->_CSS = DRAGIFYWP_HB_DIR.'/dist/css/';
		$this->_JS = DRAGIFYWP_HB_DIR.'/dist/js/';
		$this->_IMAGE = DRAGIFYWP_HB_DIR.'/img/';

		$this->render_live_builder();
		$this->render_dnd_area();

		if (isset($_GET['page']) && $_GET['page'] == 'dragifywp-header-setting-page') {
	...
	public function add_meta_boxes_callback() {
		add_meta_box('dragifywp-page-header-meta-box', 'Header Setting', array($this, 'page_header_meta_box_render'), 'page', 'side', 'low');
	}

	public function page_header_meta_box_render() {
		wp_nonce_field( basename( __FILE__ ), 'dragifywp_page_header_nonce' );
		echo "<div id='dragifywp-page-header'><page-header-metabox post-id='" . get_the_ID() . "'></page-header-metabox></div>";
	}
	...
	public function page_header_metabox_enqueue_script(){
		...
		if ('page' == get_post_type($ID)) {
			$setting = dragifywp_get_header_setting($ID);
			...
			$translation = array(
				...
				"label" => 'header',
				"items" => $headers,
				"api" => get_rest_url() . "dragifywp-header-builder/v1",
				"edit_url" => add_query_arg(array('dragifywp-hb-enable' => 1), get_admin_url()),
				"nonce" => wp_create_nonce('wp_rest'),
				"list_url" => add_query_arg(array('page' => 'dragifywp-header-setting-page'), get_admin_url(null, 'admin.php')),
			...
			wp_register_script('dragifywp_hb_page_selection_js', $this->_JS.'/page.selection.min.js', array(), DRAGIFYWP_APP_VERSION, false);
			wp_localize_script('dragifywp_hb_page_selection_js', 'dragifywp_app_header_data', $translation);
			wp_enqueue_script('dragifywp_hb_page_selection_js');

			wp_register_style('fontawesome', $this->_3RD.'/fontawesome/font-awesome.min.css',array(), '4.7.0', 'screen');
			wp_enqueue_style('fontawesome');

			wp_register_style('dragifywp_page_header', $this->_CSS.'page.selection.min.css');
			wp_enqueue_style('dragifywp_page_header');
		}
	}
	...
	public function page_save_header_meta_box_data($post_id) {
		...
		if (!isset( $_POST['dragifywp_page_header_nonce']) || !wp_verify_nonce($_POST['dragifywp_page_header_nonce'], basename(__FILE__))) {
			return;
		}
		...
		$setting = json_decode(str_replace('\\', '', $_POST['dragifywp-page-header-setting']), true);

		if (!empty($setting) && is_array($setting)) {
			update_post_meta($post_id, 'dragifywp-page-header-setting', $setting);
		}
	}
	...
	public function enqueue_scripts()
	{
		...
		$header_setting_id = dragifywp_get_header_id($data);

		if ($header_id == 'primary') {
			$options = dragifywp_get_header_options();
		...
		$custom_css = get_post_meta($header_id, '_dragifywp_hb_custom_css', true);
		$body_padding = get_post_meta($header_id, '_dragifywp_hb_body_padding', true);
		$model = get_post_meta($header_id, '_dragifywp_hb_model', true);
		...
		$redirect_links = array(
			...
			array('text' => 'Header Manager', 'link' => add_query_arg(array(
				'page' => 'dragifywp-header-setting-page'
			), get_admin_url(null, 'admin.php')), 'newTab' => false)
		);

		if ($is_singular && DragifyWPPageBuilder::post_type_is_accepted(get_post_type($post_id))) {
			$redirect_links[] = array(
				'text' => 'Page Builder',
		...
		$component = array(
			...
			'nonce' => wp_create_nonce('wp_rest'),
			'frame_url' => add_query_arg(array('dragifywp-hb-dnd-enable' => true), $frame_url),
			'header_id' => $_GET['header_id'],
			'current_menu_type' => $current_menu_type,
			'current_menu_object' => $current_menu_object,
			'edit_url' => add_query_arg(array('dragifywp-hb-enable' => true), get_admin_url()),
			...
			'message' => $message,
			'template_identify_code' => dragifywp_get_identify_code('dragifywp-hb-template'),
			'enable_debug' => DRAGIFYWP_DEBUG_MODE,
			...
			'post_id' => $post_id,
			'widgets' => DragifyWPHeaderBuilderWidgetManager::fetch()
		);
		...
		wp_register_script('dragifywp_hb_js', $this->_JS.'/bs.min.js',array('angularjs'),DRAGIFYWP_APP_VERSION,true);
		wp_localize_script('dragifywp_hb_js', 'dragifywp_hb', $component );
		wp_enqueue_script('dragifywp_hb_js');
		...
		wp_register_style('dragifywp_hb_css', $this->_CSS.'/bs.min.css', array(), DRAGIFYWP_APP_VERSION ,'all');
		wp_enqueue_style('dragifywp_hb_css');
	...
	public function admin_enqueue_scripts() {
		...
		$post_types = dragifywp_hb_get_post_types();
		...
		$data = array(
			'dragifywp_admin_hb_nonce' => wp_create_nonce('dragifywp_admin_hb'),
			'api' => get_rest_url() . 'dragifywp-header-builder/v1',
			'nonce' => wp_create_nonce('wp_rest'),
			'version' => DRAGIFYWP_APP_VERSION,
			'edit_url' => add_query_arg(array('dragifywp-hb-enable' => true), get_admin_url()),
		...
		wp_register_script('dragifywp_hb_crud_js', $this->_JS . '/crud.min.js', array('angularjs'), DRAGIFYWP_APP_VERSION, false);
		wp_localize_script('dragifywp_hb_crud_js', 'dragifywp_app_data', $data);
		wp_enqueue_script('dragifywp_hb_crud_js');

		wp_register_script('select2', $this->_3RD.'/select2.full.min.js', array('jquery'), '4.0.3' ,false);
		wp_enqueue_script('select2');

		wp_register_style('dragifywp_hb_crud_css', $this->_CSS.'/crud.min.css');
		wp_enqueue_style('dragifywp_hb_crud_css');
	...
	function builder_area()
	{
		...
		if ( isset($_GET['dragifywp-hb-enable']) && array_intersect($allowed_roles, $user->roles) && isset($_GET['header_id'])) {
			...
			if (get_post_type($header_id) == 'dragifywp_header') {
				...
				<body ng-app="dragifywp.builder.app" ng-strict-di>
					<div class="dragifywp-app" ng-controller="dragifywp_builder_app_controller" >
						<dragifywp-header-builder></dragifywp-header-builder>
	...
	public function dnd_register_script(){
		wp_register_script('dragifywp_header_iframe', $this->_JS.'/dnd.min.js',array(), DRAGIFYWP_APP_VERSION, false);
		wp_enqueue_script('dragifywp_header_iframe');

		wp_register_style('dragifywp_header_dnd', $this->_CSS.'/dnd.min.css', array(), DRAGIFYWP_APP_VERSION, 'screen');
		wp_enqueue_style('dragifywp_header_dnd');
		...
		if (!empty($options['uploaded_fonts'])) {
			...
			wp_add_inline_style('dragifywp.header.dnd', $style);
		}
		...
		if (!empty($options['typekit_fonts'])) {
			...
			foreach ($options['typekit_fonts'] as $kit_id => $kit) {
				wp_add_inline_script('dragifywp.header.iframe', str_replace('@kitid', $kit_id, $script));
			}
		}
	}

	private function render_dnd_area() {
		if ( isset($_GET['dragifywp-hb-dnd-enable'])) {
			show_admin_bar(false);
	...
	public function add_header_setting_page() {
		add_menu_page(
			...
			'Header Manager',
			'administrator',
			'dragifywp-header-setting-page',
			array($this, 'dragifywp_header_setting_page'),
			'dashicons-editor-kitchensink',
			81
		);
	}

	public function dragifywp_header_setting_page() {
		?>
		<div class="wrap" id="dragifywp-header-builder-admin" ng-app="dragifywp.app.manager" ng-controller="dragifywp_app_manager_controller" ng-strict-di>
			<app-admin></app-admin>
		</div>
		<?php
	}
	...
	public function register_rest_routes() {
		$rest_api = new DragifyWPHeaderBuilderAdminAPI();
		$rest_api->register_routes();
	}

}

new DragifyWPHeaderBuilderAdmin();