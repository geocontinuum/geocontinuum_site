...
class DragifyWPHeaderBuilderClient
{
	const DRAGIFYWP_SHORTCODE_ATTS_FILTER = 'dragifywp_hb';

	function __construct()
	{
		$this->ROOT =  DRAGIFYWP_CORE_ROOT.'/app/client';
	...
	public function enqueue_styles() {
		...
		if (isset($_GET['preview-header']) && get_post_type($_GET['preview-header']) == 'dragifywp_header') {
	...
	public function add_header_style() {
		if (!isset($_GET['dragifywp-hb-dnd-enable'])) {
			if (isset($_GET['preview-header']) && get_post_type($_GET['preview-header']) == 'dragifywp_header') {
				...
				$header_id = dragifywp_get_header_id(dragifywp_get_header_data());
			}

			$header = get_post($header_id);
			if (!empty($header)) {
				$header_style = '';
				$styles = get_post_meta($header_id, '_dragifywp_hb_styles', true);
				...
				if (!empty($styles['link']) && is_array($styles['link'])) {
					...
					foreach ($styles['link'] as $link) {
						wp_enqueue_style('dragifywp-header-link-' . $i, $link);
						$i++;
					}
				}
				...
				wp_add_inline_style('dragifywp_builder_css', $header_style);
			}
		}
	}

	public function header_preview_style(){
		wp_enqueue_style('dragifywp_header_admin', DRAGIFYWP_HB_DIR.'/dist/css/preview.min.css', array(), DRAGIFYWP_APP_VERSION ,'all');
	}

	public static function render_frontend_header() {
		$html = '';
		$data = dragifywp_get_header_data();

		if (isset($_GET['preview-header']) && get_post_type($_GET['preview-header']) == 'dragifywp_header' && (is_home() || is_page())) {
			if (is_home()) {
				$id = 0;
			} else {
				$id = get_the_ID();
			...
			if ($header_setting_id != $header_id) {
				...
				ob_start();?>
				<button class="dragifywp-bs__itemPreview__setBtn" id="y67123PnCyO8" state="ready" data-header-id="<?php echo $header_id; ?>" data-page-id="<?php echo $id; ?>">
				...
				<script type="text/javascript" id="BS4uQxW20G57">
					jQuery(window).ready(function(){
						jQuery("#y67123PnCyO8").on('click', function() {
							...
							jQuery.ajax({
								method: 'POST',
								url: '<?php echo get_rest_url(); ?>dragifywp-header-builder/v1/set-header',
								data: {
		...
		if (!empty($header_id)) {
			$html .= '<header class="dragifywp-site__header">';
			$header = get_post($header_id);
			if (!empty($header)) {
	...
	public static function render_dnd_area() {
		add_filter('edit_post_link', '__return_false');
			return '
			<header class="dragifywp-site__header" id="dragifywp-hb-dnd-side"></header>
		';
	}

	public static function render(){
		if (isset($_GET['dragifywp-hb-dnd-enable'])) {
			echo DragifyWPHeaderBuilderClient::render_dnd_area();
		} else {
			echo DragifyWPHeaderBuilderClient::render_frontend_header();
		}
	}
...
new DragifyWPHeaderBuilderClient();