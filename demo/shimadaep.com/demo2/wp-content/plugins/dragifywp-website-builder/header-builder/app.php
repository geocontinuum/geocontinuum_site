<?php
require_once 'megamenu/DragifyWPMegaMenu.php';
require_once 'widgets/widgets.php';
require_once 'DragifyWPHeaderBuilderAdminAPI.php';
require_once 'DragifyWPHeaderBuilderAdmin.php';
require_once 'DragifyWPHeaderBuilderClient.php';


function dragifywp_get_header_options() {
	$options = get_option('dragifywpHeaderOptions');

	return is_array($options) ? $options : array();
}

function dragifywp_get_header_setting($id) {
	$setting = get_post_meta($id, 'dragifywp-page-header-setting', true);
	return !empty($setting) && is_array($setting) ? $setting : array();
}

function dragifywp_get_header_id($data) {
	...
	if (!empty($header_id)) {
		...
		if (!$header || $header->post_status != 'publish' || $header->post_type != 'dragifywp_header') {
			$header_id = 0;
		}
	}
...
function dragifywp_hb_get_post_types() {
	return get_post_types(array('show_ui' => true), 'objects');
}

function dragifywp_get_header_data() {
	$data = array();
...
function dragifywp_header_builder_render(){
	DragifyWPHeaderBuilderClient::render();
}