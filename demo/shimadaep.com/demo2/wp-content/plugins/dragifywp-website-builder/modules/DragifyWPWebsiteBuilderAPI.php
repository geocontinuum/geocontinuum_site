...
class DragifyWPWebsiteBuilderAPI
{
	private $_base_api, $_save_properties;

	public function __construct() {
		$this->_base_api = 'dragifywp-website-builder/v1';
	...
	public function get_linear_icons_callback() {
		return json_decode(file_get_contents(DRAGIFYWP_CORE_ROOT . '/modules/json/linear-icons.json'));
	}

	public function get_font_awesome_callback($request_data) {
		return json_decode(file_get_contents(DRAGIFYWP_CORE_ROOT . '/modules/json/font-awesome.json'));
	}

	public function get_font_awesome_social_icons_callback($request_data) {
		return json_decode(file_get_contents(DRAGIFYWP_CORE_ROOT . '/modules/json/font-awesome-social-icons.json'));
	}
	...
	public function get_instagram_callback($request_data) {
		...
		if (dragifywp_connect_fs('')) {
			...
			if (dragifywp_exist_url($user_url)) {
				...
				if (dragifywp_exist_url($media_url)) {
	...
	public function get_permission_callback($request_data) {
		if (!current_user_can('edit_posts')) {
			return new WP_Error('dragifywp_forbidden_context', 'You are not allowed to run this function.', array('status' => 403));
		}
...
new DragifyWPWebsiteBuilderAPI();
