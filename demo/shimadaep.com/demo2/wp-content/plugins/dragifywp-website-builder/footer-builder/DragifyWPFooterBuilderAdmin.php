...
class DragifyWPFooterBuilderAdmin
{
	function __construct()
	{
		$this->_DIST = DRAGIFYWP_FB_DIR.'/dist/';
		$this->_IMAGE = DRAGIFYWP_FB_DIR.'/img/';
		...
		if (isset($_GET['page']) && $_GET['page'] == 'dragifywp-footer-setting-page') {
	...
	public function add_meta_boxes_callback() {
		add_meta_box('dragifywp-page-footer-meta-box', 'Footer Setting', array($this, 'page_footer_meta_box_render'), 'page', 'side', 'low');
	}

	public function page_footer_meta_box_render() {
		wp_nonce_field(basename(__FILE__), 'dragifywp_page_footer_nonce');
		echo "<div id='dragifywp-page-footer'><page-footer-metabox post-id='" . get_the_ID() . "'></page-footer-metabox></div>";
	}
	...
	function page_save_footer_meta_box_data($post_id){
		...
		if (!isset($_POST['dragifywp_page_footer_nonce']) || !wp_verify_nonce($_POST['dragifywp_page_footer_nonce'], basename(__FILE__))) {
			return;
		}
		...
		$setting = json_decode(str_replace('\\', '', $_POST['dragifywp-page-footer-setting']), true);

		if (!empty($setting) && is_array($setting)) {
			update_post_meta($post_id, 'dragifywp-page-footer-setting', $setting);
		}
	}
	...
	function page_footer_metabox_enqueue_script(){
		...
		if ('page' == get_post_type($ID)) {
			...
			$setting = dragifywp_get_footer_setting($ID);
			...
			$translation = array(
				...
				"items" => $footers,
				"label" => "footer",
				"api" => get_rest_url() . "dragifywp-footer-builder/v1",
				"edit_url" => add_query_arg(array('dragifywp-fb-enable' => 1), get_admin_url()),
				"nonce" => wp_create_nonce('wp_rest'),
				"enable_debug" => DRAGIFYWP_DEBUG_MODE,
				"list_url" => add_query_arg(array('page' => 'dragifywp-footer-setting-page'), get_admin_url(null, 'admin.php')),
			...
			wp_register_script('dragifywp_fb_page_selection_js', $this->_JS . '/page.selection.min.js', array(), DRAGIFYWP_APP_VERSION, false);
			wp_localize_script('dragifywp_fb_page_selection_js', 'dragifywp_app_footer_data', $translation);
			wp_enqueue_script('dragifywp_fb_page_selection_js');

			wp_register_style('fontawesome', $this->_3RD.'/fontawesome/font-awesome.min.css',array(), '4.7.0', 'screen');
			wp_enqueue_style('fontawesome');

			wp_register_style('dragifywp_fb_page_selection_css', DRAGIFYWP_FB_DIR . '/dist/css/page.selection.min.css');
			wp_enqueue_style('dragifywp_fb_page_selection_css');
		}
	}
	...
	public function register_post_types() {
		register_post_type('dragifywp_footer', array(
			'label' => 'DragifyWP Footer',
			'has_archive' => true
		));

		register_post_type('dragifywp_footer_tpl', array(
			'label' => 'DragifyWP Footer Template',
			'has_archive' => true
		));
	}
	...
	public function enqueue_scripts()
	{
		...
		$footer_setting_id = dragifywp_get_footer_id($data);

		if ($footer_id == 'primary') {
			$options = dragifywp_get_footer_options();
		...
		$custom_css = get_post_meta($footer_id, '_dragifywp_fb_custom_css', true);
		$model = get_post_meta($footer_id, '_dragifywp_fb_model', true);
		...
		$redirect_links = array(
			...
			array('text' => 'Footer Manager', 'link' => add_query_arg(array(
				'page' => 'dragifywp-footer-setting-page'
		...
		if ($is_singular && class_exists('DragifyWPPageBuilder') && DragifyWPPageBuilder::post_type_is_accepted(get_post_type($post_id))) {
			$redirect_links[] = array(
				'text' => 'Page Builder',
				'link' => add_query_arg(array(
					'dragifywp-pb-enable' => true,
					'post_id' => $post_id
		...
		$component = array(
			...
			'nonce' => wp_create_nonce('wp_rest'),
			'frame_url' => add_query_arg(array('dragifywp-fb-dnd-enable' => true), $frame_url),
			'footer_id' => $_GET['footer_id'],
			'current_menu_type' => $current_menu_type,
			'current_menu_object' => $current_menu_object,
			'edit_url' => add_query_arg(array('dragifywp-fb-enable' => true), get_admin_url()),
			...
			'message' => $message,
			'enable_debug' => DRAGIFYWP_DEBUG_MODE,
			'template_identify_code' => dragifywp_get_identify_code('dragifywp-fb-template'),
			'dragifywp_pb' => true,
			...
			'widgets' => DragifyWPFooterBuilderWidgetManager::fetch()
		);
		...
		wp_register_script('dragifywp_fb_js', $this->_JS . '/bs.min.js', array('angularjs'), DRAGIFYWP_APP_VERSION, true);
		wp_localize_script('dragifywp_fb_js', 'dragifywp_fb', $component);
		wp_enqueue_script('dragifywp_fb_js');
		...
		wp_register_style('dragifywp_fb_css', $this->_CSS.'/bs.min.css', array(), DRAGIFYWP_APP_VERSION ,'all');
		wp_enqueue_style('dragifywp_fb_css');
	...
	function builder_area()
	{
		...
		if (isset($_GET['dragifywp-fb-enable']) && array_intersect($allowed_roles, $user->roles) && isset($_GET['footer_id'])) {
			...
			if (get_post_type($footer_id) == 'dragifywp_footer') {
				...
				echo '
					<body ng-app="dragifywp.builder.app" ng-strict-di>
						<div class="dragifywp-app" ng-controller="dragifywp_builder_app_controller">
							<dragifywp-footer-builder></dragifywp-footer-builder>
	...
	public function dnd_register_script(){
		wp_enqueue_script('dragifywp.footer.dnd', $this->_DIST.'/js/dnd.min.js', array(), DRAGIFYWP_APP_VERSION ,false);
		wp_enqueue_style('dragifywp.footer.dnd', $this->_DIST.'/css/dnd.min.css', array(), DRAGIFYWP_APP_VERSION, 'screen');
		wp_enqueue_style('linearicon_min', DRAGIFYWP_CORE_ROOT.'/modules/3rd/linearicon-min/style.css',array(), '1.0.0', 'screen');
		...
		if (!empty($options['uploaded_fonts'])) {
			...
			wp_add_inline_style('dragifywp.footer.dnd', $style);
		}
		...
		if (!empty($options['typekit_fonts'])) {
			...
			foreach ($options['typekit_fonts'] as $kit_id => $kit) {
				wp_add_inline_script('dragifywp.footer.iframe', str_replace('@kitid', $kit_id, $script));
			}
		}
	}

	private function render_dnd_area() {
		if (isset($_GET['dragifywp-fb-dnd-enable'])) {
			show_admin_bar(false);
	...
	public function register_rest_routes() {
		$rest_api = new DragifyWPFooterBuilderAPI();
	...
	public function admin_enqueue_scripts() {
		...
		$post_types = dragifywp_fb_get_post_types();

		if (!empty($post_types)) {
			foreach ($post_types as $post_type) {
				if (!in_array($post_type->name, array('page', 'attachment', 'dragifywp_block'))) {
		...
		wp_register_script('dragifywp_admin_fb', $this->_JS . 'crud.min.js', array('angularjs'), DRAGIFYWP_APP_VERSION, false);
		wp_localize_script('dragifywp_admin_fb', 'dragifywp_app_data', $data);
		wp_enqueue_script('dragifywp_admin_fb');

		wp_enqueue_script('select2', $this->_3RD.'/select2.full.min.js', array('jquery'), '4.0.3' ,false);
		wp_enqueue_script('select2');

		wp_register_style('dragifywp_admin_fb', $this->_CSS.'/crud.min.css', array(), DRAGIFYWP_APP_VERSION, 'screen');
		wp_enqueue_style('dragifywp_admin_fb');
	...
	public function add_footer_setting_page() {
		add_menu_page(
			...
			'Footer Manager',
			'administrator',
			'dragifywp-footer-setting-page',
			array($this, 'dragifywp_footer_setting_page'),
			'dashicons-editor-kitchensink',
			82
		);
	}

	public function dragifywp_footer_setting_page() {
		?>
		<div class="wrap" id="dragifywp-footer-builder-admin" ng-app="dragifywp.app.manager" ng-controller="dragifywp_app_manager_controller" ng-strict-di>
			<app-admin></app-admin>
		</div>
		<?php
	}
...
new DragifyWPFooterBuilderAdmin();
