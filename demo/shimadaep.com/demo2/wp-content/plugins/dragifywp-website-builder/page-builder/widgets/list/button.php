...
add_shortcode('dragifywp_button', 'dragifywp_button_callback');
function dragifywp_button_callback($attrs, $content = null) {
    ...
    if (!empty($attrs['link']['url'])) {
        ...
        $link_before = '<a href="' . $attrs['link']['url'] . '" class="dragifywp-button ' . implode(' ', $classes) . '"' . $id . $animation . $target . '>';
        $link_after = '</a>';
    } else {
        $link_before = '<span class="dragifywp-button ' . implode(' ', $classes) . '"' . $id. $animation . '>';
        $link_after = '</span>';
    }
