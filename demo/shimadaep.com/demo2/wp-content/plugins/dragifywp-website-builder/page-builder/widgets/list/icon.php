        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "responsive_styles" => array(
                        "@id .dragifywp-icon__holder" => array(
                            "width" => "{{@model}}px"
                        )
                    )
                ),
...
ob_start();
?>
<div class="dragifywp-dnd__emptyItem" style="height:50px;" ng-if="!$ctrl.edited.attrs.icon.icon">
    <span class="dragifywp-dnd__emptyItem__label">Icon</span>
</div>

<div class="dragifywp-icon {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]" id="{{$ctrl.edited.attrs.id}}">
    <figure class="dragifywp-icon__holder">
...
add_shortcode('dragifywp_icon', 'dragifywp_icon_callback');
function dragifywp_icon_callback($attrs) {
    ...
    if (!empty($attrs['link']['url'])) {
        ...
        $before = '<a href="' . $attrs['link']['url'] . '" class="dragifywp-icon__holder" ' . $target.$animation . '>';
        $after = '</a>';
    } else {
        $before = '<figure class="dragifywp-icon__holder".'.$animation.'.>';
        $after = '</figure>';
    }

    ob_start(); ?>
    <div class="dragifywp-icon <?php echo implode(' ', $classes); ?>"<?php echo $id; ?>>
        <?php echo $before; ?>
