...
$widget = array(
    "name" => "Metro Posts",
    "tag" => "dragifywp_fw_metro_posts",
    "keywords" => ["posts","metro"],
    "native" => false,
    "icon_url" => DRAGIFYWP_FW_ROOT."/page-builder-widgets/img/metro-posts.png",
    "filter" => "other",
    "content" => [
        array(
                    ...
                    "content" => [
                        ...
                        array(
                            ...
                            "attrs" => array(
                                ...
                                "config" => array(
                                    "none" => "None",
                                    "dragifywp-sa--moveTop" => "Move Top",
                                    "dragifywp-sa--moveTopLightRotate" => "Move Top Light Rotate",
                                    "dragifywp-sa--moveTopLeft" => "Move Top Left",
                                    "dragifywp-sa--moveTopRight" => "Move Top Right",
                                    "dragifywp-sa--moveLeft" => "Move Left",
                                    "dragifywp-sa--moveRight" => "Move Right",
                                    "dragifywp-sa--fadeIn" => "Fade In",
                                    "dragifywp-sa--scaleOut" => "Scale Out"
                                )
                            ),
                            "default_value" => "dragifywp-sa--scaleOut"
                        ),
                        array(
                            "tag" => "editor-field",
                    ...
                    "content" => [
                        array(
                            ...
                            "attrs" => array(
                                ...
                                "config" => array(
                                    "options" => array(
                                        "dragifywp-blogMetro--flexStart" => "Start",
                                        "dragifywp-blogMetro--spaceBetween" => "Space",
                                        "dragifywp-blogMetro--flexEnd" => "End"
                                    )
                                )
                            ),
                            "default_value" => "dragifywp-blogMetro--flexEnd"
                        ),
                        array(
                            "tag" => "editor-field",
                        ...
                        array(
                            ...
                            "responsive_styles" => array(
                                "@id .dragifywp-blogMetro [class*='dragifywp-metro--s'] > *" => array(
                                    "paddingBottom" => "{{@model}}%"
                                ),
                                "@id .dragifywp-blogMetro [class*='dragifywp-metro--v'] > *" => array(
                                    "paddingBottom" => "{{@model*2}}%"
                                ),
                                "@id .dragifywp-blogMetro [class*='dragifywp-metro--h'] > *" => array(
                                    "paddingBottom" => "{{@model/2}}%"
                                )
                            )
                        ),
                        array(
                            ...
                            "responsive_styles" => array(
                                "@id .dragifywp-bmi__content" => array(
                                    "padding" => "{{@model}}"
                                )
                            )
                        )
                ...
                array(
                    ...
                    "content" => [
                        array(
                            ...
                            "default_value" => "#ffffff",
                            "styles" => array(
                                "@id .dragifywp-bmi__author" => array(
                                    "color" => "{{@model}}"
                                )
                            )
                        ),
                        ...
                        array(
                            ...
                            "default_value" => "#ffffff",
                            "styles" => array(
                                "@id .dragifywp-bmi__footer" => array(
                                    "color" => "{{@model}}"
                                )
                            )
                        )
                ...
                array(
                    ...
                    "content" => [
                        array(
                            ...
                            "styles" => array(
                                "@id .dragifywp-bmi__overlay" => array(
...
ob_start();
?>
<div class="dragifywp-metro-posts {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]" id="{{$ctrl.edited.attrs.id}}">
    <dragifywp-shortcode tag="dragifywp_fw_metro_posts_content" model="$ctrl.edited"
                     addition="pb='true'"
                     attrs="['num', 'start', 'categories', 'order', 'scroll_effect','arrange_content', 'paging', 'custom_style', 'size_list', 'size_list_custom']"
                     callback="[{'function':'dragifywpBindBlogLoadMore','selector':'.dragifywp-blogMetro__loadMoreBtn'},{'function':'dragifywpBindBlogInfiniteScroller','selector':'.dragifywp-blogMetro__infiniteScroller'},{'function':'dragifywpLazyImage','selector':'.dragifywp-blogMetro'}]"></div>
<?php
$widget['visual_view'] = ob_get_clean();

$manager->add($widget);

add_shortcode('dragifywp_fw_metro_posts', 'dragifywp_fw_metro_posts_callback');
function dragifywp_fw_metro_posts_callback($attrs) {
    $attrs = shortcode_atts(array(
        ...
        'paging' => 'none',
        'custom_style' => 'false',
        'scroll_effect' => 'dragifywp-sa--scaleOut',
        'arrange_content' => 'dragifywp-blogMetro--spaceBetween',
        'size_list' => 's14',
    ...
    ob_start(); ?>
    <div class="dragifywp-metro-posts <?php echo implode(' ', $classes); ?>"<?php echo $id; ?>>
        <?php echo do_shortcode('[dragifywp_fw_metro_posts_content num="' . $attrs['num'] . '" start="' . $attrs['start'] . '" order="' . $attrs['order'] . '" categories="' . $attrs['categories'] . '" paging="' . $attrs['paging'] . '" scroll_effect="' . $attrs['scroll_effect'] . '" custom_style="' . $attrs['custom_style'] . '" arrange_content="' . $attrs['arrange_content'] . '" size_list="' . $attrs['size_list'] . '" size_list_custom="' . $attrs['size_list_custom'] . '"]'); ?>
    </div>
    <?php
    return ob_get_clean();
}

add_shortcode('dragifywp_fw_metro_posts_content', 'dragifywp_fw_metro_posts_content_callback');
function dragifywp_fw_metro_posts_content_callback($attrs) {
    $attrs = shortcode_atts(array(
        'num' => 6,
        'order' => 'DESC',
        'categories' => '',
    ...
    if (dragifywp_fw_theme_to_boolean($attrs['custom_style'])) {
        $scroll_effect = $attrs['scroll_effect'];
        $arrange_content = $attrs['arrange_content'];
        $sizes = dragifywp_fw_get_metro_sizes($attrs['size_list'], $attrs['size_list_custom']);
    } else {
    ...
    $class_arrange_content = !empty($arrange_content) ? ' '.$arrange_content : ' dragifywp-blogMetro--spaceBetween';
    ...
    <div class="dragifywp-blog dragifywp-blogMetro<?php echo $class_arrange_content; ?>"<?php echo $data_animate_scroll;?>>
        ...
        if (!empty($posts)) :
            foreach ($posts as $post) :
                ...
                dragifywp_fw_metro_post_item_render(array('scroll_effect' => $scroll_effect), array($metro_class));
            endforeach;
    ...
    <?php if (dragifywp_fw_theme_to_boolean($attrs['pb'])) : ?>
        <img src="<?php echo DRAGIFYWP_FW_ROOT . '/img/article.png'; ?>" style="display: none;" onload="jQuery(this).prev().dragifywpBindMetroMasonry()" />
    <?php endif; ?>
