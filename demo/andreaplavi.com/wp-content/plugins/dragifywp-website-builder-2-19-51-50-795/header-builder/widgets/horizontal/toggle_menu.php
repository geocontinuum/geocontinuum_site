                ...
                array(
                    ...
                    "content" => [
                        array(
                            ...
                            "default_value" => 20,
                            "styles" => array(
                                "@id a, @id .dragifywp-menu-column-title" => array(
                                    "fontSize" => "{{@model}}px"
                                )
                            )
                        ),
                        ...
                        array(
                            ...
                            "directives" => [
                                array(
                                    ...
                                    "attrs" => array(
                                        "selector" => "@id, @id .dragifywp-menu-column-title"
                                    )
                                )
                            ]
                        )
                ...
                array(
                    ...
                    "content" => [
                        ...
                        array(
                            ...
                            "default_value" => $manager::$colors["primary_context"],
                            "styles" => array(
                                "@id a:hover, @id .dragifywp-menu-column-title:hover" => array(
                                    "backgroundColor" => "{{@model}}"
                                )
                            )
                        ),
                        ...
                        array(
                            ...
                            "default_value" => $manager::$colors["primary_context"],
                            "styles" => array(
                                "@id li.dragifywp-menu-current-menu-item > a,@id li.dragifywp-menu-current-menu-item > .dragifywp-menu-column-title" => array(
                                    "backgroundColor" => "{{@model}}"
                                )
                            )
                        ),
                        ...
                        array(
                            ...
                            "default_value" => "none",
                            "styles" => array(
                                "@id  li, @id .dragifywpmenu-item-has-children, @id .dragifywp-menu-column-title" => array(
        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "default_value" => 22,
                    "styles" => array(
                        "@id__btn .dragifywp-header__burgerBtn__icon" => array(
                            "width" => "{{@model}}px"
                        )
                    )
                ),
...
<div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--h" style="width:140px;" ng-if="!$ctrl.edited.attrs.menu_id">
    <span class="dragifywp-dnd__emptyItem__label">Toggle Menu</span>
</div>

<div class="dragifywp-header__burgerBtn {{::$ctrl.getID()}}__btn"
     ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden,
                {'dragifywp-header__burgerBtn--active':$ctrl.edited.enable, 'dragifywp-header__burgerBtn--rounded-corners':$ctrl.edited.attrs.btn_rounded_corners,
                        'dragifywp-header__burgerBtn--allow-cross':$ctrl.edited.attrs.cross_icon}]">
    <div class="dragifywp-header__burgerBtn__icon">
        <span></span>
        <span></span>
        <span></span>
    </div>
...
function lhb_h_toggle_menu_callback($attrs) {
    ...
    if (dragifywp_to_boolean($attrs['btn_rounded_corners'])) {
        $classes[] = 'dragifywp-header__burgerBtn--rounded-corners';
    }

    if (dragifywp_to_boolean($attrs['cross_icon'])) {
        $classes[] = 'dragifywp-header__burgerBtn--allow-cross';
    }
    ...
    if (dragifywp_to_boolean($attrs['one_page'])) {
        $menu_classes[] = 'dragifywp-onePageMenu';
    }

    $menu_classes[] = $attrs['class_id'];

    $menu_content =dragifywp_get_menu_content($attrs['menu_id'], 'toggle');

    $html = '<div class="dragifywp-header__burgerBtn ' . implode(' ', $classes) . '" id="'.$attrs['class_id'].'">';
    $html .= '<div class="dragifywp-header__burgerBtn__icon">';
    $html .= '<span></span><span></span><span></span>';
    $html .= '</div>';
    $html .= '</div>';
    $html .= '<div class="dragifywp-header__toggleMenu ' . implode(' ', $menu_classes) . '" style="display: none!important">';
