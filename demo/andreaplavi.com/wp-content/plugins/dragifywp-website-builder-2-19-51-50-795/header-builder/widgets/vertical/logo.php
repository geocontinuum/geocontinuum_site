...
ob_start();
?>
<div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--v" style="height:100px;" ng-if="!$ctrl.edited.attrs.img.url">
    <span class="dragifywp-dnd__emptyItem__label">Logo</span>
</div>

<a class="dragifywp-aside__logo {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.hidden, $ctrl.edited.attrs.class]"></a>
<?php
...
function lhb_v_logo_callback($attrs) {
    ...
    if ($link) {
        ...
        $before_link = '<a href="' . $link . '" class="dragifywp-aside__logo ' . implode(' ', $classes) . '"' . $target . '>';
        $after_link = '</a>';
    } else {
        $before_link = '<span class="dragifywp-aside__logo ' . implode(' ', $classes) . '">';
        $after_link = '</span>';
    }
