...
abstract class DragifyWPMenuItem {
	...
	function get_submenu_wrap_start() {
		$class = 'dragifywp-menu-submenu dragifywp-menu-submenu-id-' . $this->item->ID;

		if ($this->depth == 0) {
			$class .= ' dragifywp-menu-dropdown';
		}

		if (isset($this->item->has_row) && $this->item->has_row) {
			$class .= ' dragifywp-menu-section';
		}
		...
		foreach($this->classes as $k => $class){
			if($class) {
				if(substr($class, 0, 4) == 'menu') {
					$this->classes[$k] = 'dragifywp-' . $class;
				} else {
					$this->classes[$k] = 'dragifywp-menu-' . $class;
				}
			}
		}
	}
	...
	function add_class_level(){
		$this->classes[] = 'dragifywp-menu-item-level-' . $this->depth;
	}
	...
	function get_anchor_atts() {
		$atts = array();
		$atts['class']	= 'dragifywp-menu-anchor';
	...
	function get_anchor($atts) {
		...
		// Icon
		$icon = '';
		$icon_class = get_post_meta($this->item->ID, '_dragifywp_menu_item_icon', true);
		...
		if ($this->depth > 0) {
			$emphasis_label = sanitize_text_field(get_post_meta($this->id, '_dragifywp_menu_item_emphasis', true));
		...
		// Title
		$title = '<span class="dragifywp-menu-anchor-title">';
