...
function dragifywp_get_identify_code($text){
    return md5($text . '-dragifywp-' . $text);
}

function dragifywp_to_boolean($val) {
    return filter_var($val, FILTER_VALIDATE_BOOLEAN);
}

function dragifywp_get_option() {
    $options = get_option('dragifywpOptions');
    return !empty($options) && is_array($options) ? $options : array();
}

function dragifywp_get_menu_content($menu_id, $type = '', $depth = 0) {
...
function dragifywp_get_attachment_srcset($attachment_id) {
    $srcset = '';
...
function dragifywp_get_attachment_id_from_url($url) {
    global $wpdb;
...
function dragifywp_get_image_sizes($data){
    $sizes = '';
...
function dragifywp_form_res_class($obj, $prefix, $suffix='') {
    $cl = '';
...
function dragifywp_form_parallax_data($obj) {
    if(dragifywp_to_boolean($obj['enable'])){
...
function dragifywp_form_animation_data($obj) {
    if(dragifywp_to_boolean($obj['enable'])){
...
function dragifywp_get_uploaded_image_url_queries($image, $method = 'load') {
    $html = '';
    if (!empty($image['url'])) {
