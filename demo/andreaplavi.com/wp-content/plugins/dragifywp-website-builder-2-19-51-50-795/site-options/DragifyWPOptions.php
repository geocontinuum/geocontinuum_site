...
class DragifyWPOptions {
	...
	public function __construct()
	{
		...
		$this->_options = dragifywp_get_option();
	...
	public function register_post_types() {
		register_post_type('dragifywp_op_tpl', array(
				'label' => 'DragifyWP Option Templates',
				'has_archive' => true
		));
	}

	public function activate_plugin_callback() {
		if (get_option('dragifywpOptions') === false) {
			$data = json_decode(file_get_contents(DRAGIFYWP_SO_DIR . '/initial_data.json'), true);
			if (json_last_error() === JSON_ERROR_NONE) {
				if (!empty($data['options'])) {
					update_option('dragifywpOptions', $data['options']);
				}

				if (!empty($data['styles'])) {
					update_option('dragifywpDynamicStyles', $data['styles']);
				}
			}
		}
	}
	...
	public function add_admin_menu() {
		...
		add_submenu_page(
				...
				'Live Customizer',
				'edit_theme_options',
				'dragifywp-live-customizer',
				array($this, 'render_front_page'));
	}

	public function render_backend_page(){
		return DragifyWPBackendOptions::render_options_page();
	}
	...
	public function add_toolbar_menu($wp_admin_bar) {
		...
		$wp_admin_bar->add_node(array(
				'id' => 'dragifywp-options',
				'title' => 'Site Options'
		));

		$wp_admin_bar->add_node(array(
				'id' => 'dragifywp-backend-options',
				'title' => 'Site Options',
				'parent' => 'dragifywp-options',
				'href' => add_query_arg(array(
						'page' => 'dragifywp-options'
				), get_admin_url(null, 'admin.php'))
		));

		$args = array('page' => 'dragifywp-live-customizer');
	...
	public function enqueue_scripts_styles() {
		$dynamic_styles = get_option('dragifywpDynamicStyles');
		...
		if($styles){
			wp_add_inline_style('dragifywp_builder_css', $styles);
		}
