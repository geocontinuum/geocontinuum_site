...
ob_start();
?>
<div class="dragifywp-dnd__emptyItem" style="height:50px;" ng-if="!$ctrl.edited.attrs.image.url && $ctrl.edited.attrs.icon == 'image'">
    <span class="dragifywp-dnd__emptyItem__label">App Store</span>
</div>

<a class="dragifywp-appStoreBadge {{::$ctrl.getID()}}" href="#" ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]" id="{{$ctrl.edited.attrs.id}}">
...
add_shortcode('dragifywp_app_store_badge', 'dragifywp_app_store_badge_callback');
function dragifywp_app_store_badge_callback($attrs, $content = null) {
    ...
    if (!empty($attrs['link']['url'])) {
        ...
        $link_before = '<a href="' . $attrs['link']['url'] . '" class="dragifywp-appStoreBadge ' . implode(' ', $classes) . '"' . $id . $animation . $target . '>';
        $link_after = '</a>';
    } else {
        $link_before = '<span class="dragifywp-appStoreBadge ' . implode(' ', $classes) . '"' . $id . $animation . '>';
        $link_after = '</span>';
    }
    ...
    if ($attrs['icon'] == 'image') {
        ...
        echo '<img src="' . DRAGIFYWP_CORE_ROOT . '/client/img/badge/' . $attrs['icon'] . '.png"/>';
    }
