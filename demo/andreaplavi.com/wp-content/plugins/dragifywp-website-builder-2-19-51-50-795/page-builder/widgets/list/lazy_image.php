        ...
        array(
            ...
            "content" => [
                array(
                    ...
                    "styles" => array(
                        "@id .dragifywp-zImage__img" => array(
...
<div class="dragifywp-dnd__emptyItem" style="height:100px;" ng-if="!$ctrl.edited.attrs.image.url">
    <span class="dragifywp-dnd__emptyItem__label">Lazy Image</span>
</div>

<div class="dragifywp-zImage__container {{::$ctrl.getID()}}">
    <div class="dragifywp-zImage" id="{{$ctrl.edited.attrs.id}}" ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]">
        <div class="dragifywp-zImage__img"></div>
    </div>
</div>
<?php
...
add_shortcode('dragifywp_lazy_image', 'dragifywp_lazy_image_callback');
function dragifywp_lazy_image_callback($attrs) {
...
//	dragifywp_debug($attrs['image']);
    $attrs['image'] = json_decode($attrs['image'], true);

    ob_start(); ?>
    <div class="dragifywp-zImage__container <?php echo implode(' ', $classes) ?>">
        <?php echo $link_before; ?>
        <div class="dragifywp-zImage"<?php echo $animation.$id; ?>>
            <div class="dragifywp-zImage__img" style="padding-bottom: <?php echo $attrs['image']['padding'].'%'; ?>">
                <?php echo dragifywp_get_uploaded_image_url_queries($attrs['image'], $attrs['load_method']); ?>
            </div>
        </div>
        <?php echo $link_after; ?>
    </div>
