...
$widget = array(
    ...
    "tag" => "dragifywp_fw_classic_grid_posts",
    "keywords" => ["posts","classic","grid"],
    "native" => false,
    "icon_url" => DRAGIFYWP_FW_ROOT."/page-builder-widgets/img/classic-grid-posts.png",
    "filter" => "other",
    "content" => [
        array(
                ...
                array(
                    ...
                    "content" => [
                        ...
                        array(
                            ...
                            "default_value" => 15,
                            "styles" => array(
                                "@id .dragifywp-bcgi" => array(
                                    "padding" => "0 {{@model}}px"
                                ),
                                "@id .dragifywp-blogClassicGrid__wrap" => array(
                                    "margin" => "0 -{{@model}}px"
                                ),
                                "@id .dragifywp-blogClassicGrid" => array(
                                    "margin" => "0 -{{@model}}px",
                                    "padding" => "0 {{@model}}px"
                                )
                            )
                ...
                array(
                    ...
                    "content" => [
                        array(
                            ...
                            "responsive_styles" => array(
                                "@id .dragifywp-bcgi__content" => array(
                                    "margin" => "{{@model}}"
                                )
                            )
                        )
                ...
                array(
                    ...
                    "content" => [
                        array(
                            ...
                            "styles" => array(
                                "@id .dragifywp-bcgi__cats a" => array(
                                    "color" => "{{@model}}"
                                )
                            )
                        ),
                        array(
                            ...
                            "default_value" => $manager::$colors["desc"],
                            "styles" => array(
                                "@id .dragifywp-bcgi__author" => array(
                                    "color" => "{{@model}}"
                                )
                            )
                        )
                ...
                array(
                    ...
                    "content" => [
                        array(
                            ...
                            "responsive_styles" => array(
                                "@id .dragifywp-bcgi__title" => array(
                                    "fontSize" => "{{@model}}px"
                                )
                            )
                        ),
                        array(
                            ...
                            "styles" => array(
                                "@id .dragifywp-bcgi__title a" => array(
                                    "color" => "{{@model}}"
                                )
                            )
                        ),
                        array(
                            ...
                            "styles" => array(
                                "@id .dragifywp-bcgi__title:hover a" => array(
                                    "color" => "{{@model}}"
                                )
                            )
                        ),
                        ...
                        array(
                            ...
                            "content" => [
                                array(
                                    ...
                                    "responsive_styles" => array(
                                        "@id .dragifywp-bcgi__excerpt" => array(
                                            "fontSize" => "{{@model}}px"
                                        )
                                    )
                                ),
...
ob_start();
?>
<div class="dragifywp-classic-grid-posts {{::$ctrl.getID()}}"
     ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]"
     id="{{$ctrl.edited.attrs.id}}">
    <dragifywp-shortcode tag="dragifywp_fw_classic_grid_posts_content" model="$ctrl.edited"
                     attrs="['num', 'start', 'categories', 'order', 'excerpt_enable', 'paging', 'custom_style', 'column']"
                     callback="[{'function':'dragifywpLazyImage','selector':'.dragifywp-blogClassicGrid'}]"></div>
<?php
...
add_shortcode('dragifywp_fw_classic_grid_posts', 'dragifywp_fw_classic_grid_posts_callback');
function dragifywp_fw_classic_grid_posts_callback($attrs) {
    ...
    ob_start(); ?>
    <div class="dragifywp-classic-grid-posts <?php echo implode(' ', $classes); ?>"<?php echo $id; ?>>
        <?php echo do_shortcode('[dragifywp_fw_classic_grid_posts_content num="' . $attrs['num'] . '" column="' . $attrs['column'] . '" start="' . $attrs['start'] . '" order="' . $attrs['order'] . '" categories="' . $attrs['categories'] . '" paging="' . $attrs['paging'] . '" custom_style="' . $attrs['custom_style'] . '" excerpt_enable="' . $attrs['excerpt_enable'] . '"]'); ?>
    </div>
    <?php
    return ob_get_clean();
}

add_shortcode('dragifywp_fw_classic_grid_posts_content', 'dragifywp_fw_classic_grid_posts_content_callback');
function dragifywp_fw_classic_grid_posts_content_callback($attrs) {
    $attrs = shortcode_atts(array(
        'num' => 6,
        'order' => 'DESC',
        'categories' => '',
    ...
    if (dragifywp_fw_theme_to_boolean($attrs['custom_style'])) {
        $excerpt_enable = dragifywp_fw_theme_to_boolean($attrs['excerpt_enable']);
        $column = $attrs['column'];
    } else {
    ...
    ob_start(); ?>
    <div class="dragifywp-blog dragifywp-blogClassicGrid" data-column="<?php echo $column; ?>">
    ...
    if ($attrs['paging'] != 'none' && $has_paging) {
        ...
        switch($attrs['paging']):
            case 'button':
                dragifywp_fw_load_more_button_render(array(
                    'settings' => array(
                        'limit' => $attrs['num'],
                ...
                break;
            case 'infinite':
                dragifywp_fw_infinite_scroller_render(array(
                    'settings' => array(
                        'limit' => $attrs['num'],
                        'offset' => $offset + $attrs['num'],
                        'orderby' => 'date',
