        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "styles" => array(
                        "@id.dragifywp-header--background-gradient" => array(
...
ob_start();
?>
<div class="dragifywp-header dragifywp-hb-menu {{::$ctrl.getID()}}"
     data-scroll-class="{{$ctrl.nav.attrs.scroll_class}}"
     header-scroll-handler="$ctrl.nav.attrs"
     ng-class="[$ctrl.nav.attrs.hidden,
                            'dragifywp-header--background-' + $ctrl.nav.attrs.bg_style,
                            {'dragifywp-header--fixed':$ctrl.nav.attrs.fixed,
                            'dragifywp-header--fixedBottom': $ctrl.nav.attrs.fixed && $ctrl.nav.attrs.fixed_bottom,
                            'dragifywp-header--absolute':$ctrl.nav.attrs.absolute,
                            'dragifywp-header--fixed--delay':$ctrl.nav.attrs.scroll_delay,
                            'dragifywp-dnd__editedNav' : $ctrl.nav.kept,
                            'dragifywp-header--parallax':$ctrl.nav.attrs.parallax}]"
     data-show="{{$ctrl.nav.attrs.show_at}}">

    <div class="dragifywp-dnd__visual dragifywp-dnd__visual--bold dragifywp-dnd__visual--nav lvh__{{::$ctrl.service.getID($ctrl.nav)}}" ng-click="$ctrl.edit()" dragifywp-detect-hover-node-horizontal="$ctrl.nav"></div>

    <div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--hNav" ng-if="!$ctrl.nav.content.length">
        <span class="dragifywp-dnd__emptyItem__label">Horizontal Nav</span>
    </div>

    <horizontal-section-list nav="$ctrl.nav"></horizontal-section-list>
</div>
<div class="dragifywp-header__fixed-block" ng-class="$ctrl.nav.attrs.hidden" ng-show="$ctrl.nav.attrs.fixed"></div>
<?php
...
function lhb_horizontal_nav_callback($attrs, $content = null) {
    ...
    $classes = array(
        $attrs['class_id'],
        'dragifywp-header--background-' . $attrs['bg_style']
    );
    ...
    if (dragifywp_to_boolean($attrs['fixed'])) {
        $classes[] = 'dragifywp-header--fixed';
        $fixed_hidden = $attrs['hidden'] ? ' '.$attrs['hidden'] : '';
        $fixed_html = '<div class="dragifywp-header__fixed-block'.$fixed_hidden.'"></div>';

        if (dragifywp_to_boolean($attrs['fixed_bottom'])) {
            $classes[] = 'dragifywp-header--fixedBottom';
        }

        if (dragifywp_to_boolean($attrs['scroll_delay'])) {
            $classes[] = 'dragifywp-header--fixed--delay';
        }
    }

    if (dragifywp_to_boolean($attrs['absolute'])) {
        $classes[] = 'dragifywp-header--absolute';
    }
    ...
    ob_start(); ?>
    <div class="dragifywp-header <?php echo implode(' ', $classes); ?>"<?php echo $datas; ?>>
