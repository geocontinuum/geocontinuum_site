        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "styles" => array(
                        "@id li.dragifywp-menu-current-menu-item a" => array(
                            "color" => "{{@model}}"
                        )
                    )
                ),
                ...
                array(
                    ...
                    "content" => [
                        ...
                        array(
                            ...
                            "default_value" => $manager::$colors["primary"],
                            "styles" => array(
                                "@id[class*='--al-'] li.dragifywp-menu-current-menu-item a:before,@id[class*='--al-'] a:hover:before" => array(
                                    "backgroundColor" => "{{@model}}"
                                )
                            )
                        )
...
<div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--h" style="width:140px;" ng-if="!$ctrl.edited.attrs.menu_id">
    <span class="dragifywp-dnd__emptyItem__label">1LV Menu</span>
</div>

<div class="dragifywp-header__oneLvMenu {{::$ctrl.getID()}}"
     ng-class="[{'dragifywp-header__oneLvMenu--noEdge' : $ctrl.edited.attrs.no_edge}, 'dragifywp-header__oneLvMenu--al-' + $ctrl.edited.attrs.active_line, $ctrl.edited.attrs.hidden, $ctrl.edited.attrs.class]" dragifywp-onePageMenu model="$ctrl.edited.attrs.one_page">
...
function lhb_h_one_level_menu_callback($attrs) {
    ...
    if (dragifywp_to_boolean($attrs['no_edge'])) {
        $classes[] = 'dragifywp-header__oneLvMenu--noEdge';
    }

    if ($attrs['active_line'] != 'none') {
        $classes[] = 'dragifywp-header__oneLvMenu--al-' . $attrs['active_line'];
    }

    $classes[] = $attrs['class_id'];

    $html = '<div class="dragifywp-header__oneLvMenu ' . implode(' ', $classes) . '">';
    $html .=dragifywp_get_menu_content($attrs['menu_id'], 1);
    $html .= '</div>';
