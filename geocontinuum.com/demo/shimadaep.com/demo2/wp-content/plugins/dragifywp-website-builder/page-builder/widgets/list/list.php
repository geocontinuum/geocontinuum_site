...
<div class="dragifywp-dnd__emptyItem" style="height:100px" ng-if="$ctrl.edited.content.length == 0">
    <span class="dragifywp-dnd__emptyItem__label">List</span>
</div>

<ul class="dragifywp-list {{::$ctrl.getID()}}" selector=".dragifywp-list__item"
    ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden, 'dragifywp-list--' + $ctrl.edited.attrs.justify, {'dragifywp-list--reverse':$ctrl.edited.attrs.reverse}]" id="{{$ctrl.edited.attrs.id}}">
    <li class="dragifywp-list__item" ng-repeat="item in $ctrl.edited.content track by $index">
        <i ng-class="item.attrs.icon.source + ' ' + item.attrs.icon.source + '-' + item.attrs.icon.icon"></i>
        <span dragifywp-editable model="item" config="{target:'attrs.text', editorType:'3'}" ng-bind-html="item.attrs.text | trustashtml"></span>
    </li>
</ul>
<?php
...
add_shortcode('dragifywp_list', 'dragifywp_list_callback');
function dragifywp_list_callback($attrs, $content = null) {
    ...
    $attrs['animation']['selector'] = '.dragifywp-list__item';
    $animation = dragifywp_form_animation_data($attrs['animation']);

    $classes = array();

    if(dragifywp_to_boolean($attrs['reverse'])){
        $classes[] = 'dragifywp-list--reverse';
    }

    $classes[] = 'dragifywp-list--'.$attrs['justify'];
    ...
    ob_start(); ?>
    <ul class="dragifywp-list <?php echo implode(' ', $classes); ?>"<?php echo $id; ?><?php echo $animation; ?>>
...
add_shortcode('dragifywp_list_item', 'dragifywp_list_item_callback');
function dragifywp_list_item_callback($attrs, $content = null) {
    $attrs = shortcode_atts(array(
        'class_id' => '',
        'text' => '',
    ...
    ob_start(); ?>
    <li class="dragifywp-list__item <?php echo $attrs['class_id']; ?>">
