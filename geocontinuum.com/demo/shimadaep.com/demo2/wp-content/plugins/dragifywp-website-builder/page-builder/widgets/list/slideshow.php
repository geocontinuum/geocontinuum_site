        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "responsive_styles" => array(
                        "@id .dragifywp-slideshow__content" => array(
                            "maxWidth" => "{{@model}}"
                        )
                    )
                ),
                ...
                array(
                    ...
                    "default_value" => "",
                    "styles" => array(
                        "@id .dragifywp-slideshow__item:before" => array(
        ...
        array(
            ...
            "content" => [
                array(
                    ...
                    "responsive_styles" => array(
                        "@id .dragifywp-slideshow__heading" => array(
                            "fontSize" => "{{@model}}px"
                        )
                    )
                ),
        ...
        array(
            ...
            "content" => [
                array(
                    ...
                    "responsive_styles" => array(
                        "@id .dragifywp-slideshow__desc" => array(
                            "fontSize" => "{{@model}}px"
                        )
                    )
                ),
        ...
        array(
            ...
            "content" => [
                array(
                    ...
                    "responsive_styles" => array(
                        "@id .dragifywp-slideshow__btn" => array(
                            "fontSize" => "{{@model}}px"
                        )
                    )
                ),
                ...
                array(
                    ...
                    "default_value" => $manager::$colors["primary"],
                    "styles" => array(
                        "@id .dragifywp-slideshow__btn--left" => array(
                            "backgroundColor" => "{{@model}}"
                        )
                    )
                ),
                ...
                array(
                    ...
                    "default_value" => "#ffffff",
                    "styles" => array(
                        "@id .dragifywp-slideshow__btn--right" => array(
                            "backgroundColor" => "{{@model}}"
                        )
                    )
                ),
...
<div class="dragifywp-dnd__emptyItem" style="height:200px" ng-if="$ctrl.edited.content.length == 0">
    <span class="dragifywp-dnd__emptyItem__label">Slideshow</span>
</div>

<div class="dragifywp-slideshow {{::$ctrl.getID()}}"
     ng-class="[$ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]"
     id="{{$ctrl.edited.attrs.id}}">
    <div class="dragifywp-carousel owl-carousel" dragifywp-slideshow list="$ctrl.edited.content"
        ...
        <div class="dragifywp-slideshow__item" ng-repeat="item in $ctrl.edited.content track by $index" ng-style="{'background-image' : 'url(' + item.attrs.image.url + ')'}">
            <div class="dragifywp-slideshow__content">
                <h4 class="dragifywp-slideshow__heading" ng-bind-html="item.attrs.heading"></h4>
                <p class="dragifywp-slideshow__desc" ng-bind-html="item.attrs.desc"></p>
                <div class="dragifywp-slideshow__btnContainer">
                    <a class="dragifywp-slideshow__btn dragifywp-slideshow__btn--left" ng-show="item.attrs.btn_left_text" ng-bind-html="item.attrs.btn_left_text"></a>
                    <a class="dragifywp-slideshow__btn dragifywp-slideshow__btn--right" ng-show="item.attrs.btn_right_text" ng-bind-html="item.attrs.btn_right_text"></a>
                </div>
            </div>
        </div>
    </div>
...
add_shortcode('dragifywp_slideshow', 'dragifywp_slideshow_callback');
function dragifywp_slideshow_callback($attrs, $content = null) {
    ...
    ob_start(); ?>
    <div class="dragifywp-slideshow <?php echo implode(' ', $classes); ?>"<?php echo $id; ?>>
...
add_shortcode('dragifywp_slide', 'dragifywp_slide_callback');
function dragifywp_slide_callback($attrs) {
    $attrs = shortcode_atts(array(
        'class_id' => '',
        'heading' => DragifyWPPageBuilderClient::DRAGIFYWP_DUMB_HEADING,
    ...
    ob_start(); ?>
    <div class="dragifywp-slideshow__item <?php echo $attrs['class_id']; ?>">
        <div class="dragifywp-slideshow__content">
            <h4 class="dragifywp-slideshow__heading"><?php echo $attrs['heading']; ?></h4>
            <p class="dragifywp-slideshow__desc"><?php echo $attrs['desc']; ?></p>
            <div class="dragifywp-slideshow__btnContainer">
                <?php
                if($attrs['btn_left_text']){
                    ...
                    if (!empty($attrs['btn_left_link']['url'])) {
                        $target = dragifywp_to_boolean($attrs['btn_left_link']['new_tab']) ? ' target="_blank"' : '';
                        $link_before = '<a href="' . $attrs['btn_left_link']['url'] . '" class="dragifywp-slideshow__btn dragifywp-slideshow__btn--left"' . $target . '>';
                        $link_after = '</a>';
                    } else {
                        $link_before = '<span class="dragifywp-slideshow__btn dragifywp-slideshow__btn--left">';
                        $link_after = '</span>';
                    }
                ...
                if($attrs['btn_right_text']){
                    ...
                    if (!empty($attrs['btn_right_link']['url'])) {
                        $target = dragifywp_to_boolean($attrs['btn_right_link']['new_tab']) ? ' target="_blank"' : '';
                        $link_before = '<a href="' . $attrs['btn_right_link']['url'] . '" class="dragifywp-slideshow__btn dragifywp-slideshow__btn--right"' . $target . '>';
                        $link_after = '</a>';
                    } else {
                        $link_before = '<span class="dragifywp-slideshow__btn dragifywp-slideshow__btn--right">';
                        $link_after = '</span>';
                    }
