...
class DragifyWPFooterBuilderClient
{
	...
	const DRAGIFYWP_SHORTCODE_ATTS_FILTER = 'dragifywp_fb';
	...
	public function enqueue_styles() {
		...
		if (isset($_GET['preview-footer']) && get_post_type($_GET['preview-footer']) == 'dragifywp_footer') {
			add_action('wp_enqueue_scripts', array($this, "footer_preview_style"));
		}
	}

	public function footer_preview_style(){
		wp_enqueue_style('dragifywp_footer_admin', DRAGIFYWP_FB_DIR.'/dist/css/preview.min.css', array(), DRAGIFYWP_APP_VERSION ,'all');
	}
	...
	public function add_footer_style() {
		if (!isset($_GET['dragifywp-fb-dnd-enable'])) {
			if (isset($_GET['preview-footer']) && get_post_type($_GET['preview-footer']) == 'dragifywp_footer') {
				...
				$footer_id = dragifywp_get_footer_id(dragifywp_get_footer_data());
			}

			$footer = get_post($footer_id);
			if (!empty($footer)) {
				$footer_style = '';
				$styles = get_post_meta($footer_id, '_dragifywp_fb_styles', true);
				...
				if (!empty($styles['link']) && is_array($styles['link'])) {
					...
					foreach ($styles['link'] as $link) {
						wp_enqueue_style('dragifywp-footer-link-' . $i, $link);
						$i++;
					}
				}
				...
				wp_add_inline_style('dragifywp_builder_css', $footer_style);
			}
		}
	}
	...
	public static function render_generate_footer() {
		$html = '';
		$data = dragifywp_get_footer_data();

		if (isset($_GET['preview-footer']) && get_post_type($_GET['preview-footer']) == 'dragifywp_footer' && (is_home() || is_page())) {
			if (is_home()) {
				$id = 0;
			} else {
				$id = get_the_ID();
			...
			$html .= '<div class="dragifywp-bs__itemPreview">';
			...
			if ($footer_setting_id != $footer_id) {
				...
				ob_start();?>
				<button class="dragifywp-bs__itemPreview__setBtn" id="y67123PnCyO8" state="ready" data-footer-id="<?php echo $footer_id; ?>" data-page-id="<?php echo $id; ?>">
					<span>Apply Footer</span>
					<span><i class="dragifywp-bs__itemPreview__preloader"></i><small>Applying</small></span>
					<span><i class="fa fa-check"></i> Applied</span>
				</button>
				<div class="dragifywp-bs__itemPreview__message">
				...
				<script type="text/javascript" id="BS4uQxW20G57">
					jQuery(window).ready(function(){
						jQuery("#y67123PnCyO8").on('click', function() {
							...
							jQuery.ajax({
								method: 'POST',
								url: '<?php echo get_rest_url(); ?>dragifywp-footer-builder/v1/set-footer',
								data: {
			...
			$edit_link = add_query_arg(array(
				'dragifywp-fb-enable' => 1,
				'post_id' => $id,
				'footer_id' => $footer_id
			), get_admin_url());

			$html .= '</div>';
			$html .= '<a class="dragifywp-bs__itemPreview__editBtn" href="' . $edit_link . '"><span class="lnr lnr-pencil"></span></a>';
		} else {
			$footer_id = dragifywp_get_footer_id($data);
		}

		if (!empty($footer_id)) {
			$html .= '<footer class="dragifywp-site__footer">';
			$footer = get_post($footer_id);
			if (!empty($footer)) {
	...
	public static function render_fb_dnd_area() {
		...
		return '
			<footer class="dragifywp-site__footer" id="dragifywp-fb-dnd-area"></footer>
		';
	}
...
new DragifyWPFooterBuilderClient();