...
ob_start();
?>
<div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--h" style="height:80px;" ng-if="!$ctrl.edited.attrs.img.url">
    <span class="dragifywp-dnd__emptyItem__label">Logo</span>
</div>

<a class="dragifywp-logo {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.hidden, $ctrl.edited.attrs.class]">
    <img class="dragifywp-logo__image" ng-src="{{$ctrl.edited.attrs.img.url}}"/>
</a>
<?php
...
function lfb_logo_callback($attrs) {
    ...
    if ($link) {
        ...
        $before_link = '<a href="' . $link . '" class="dragifywp-logo ' . implode(' ', $classes) . '"' . $target . '>';
        $after_link = '</a>';
    } else {
        $before_link = '<span class="dragifywp-logo ' . implode(' ', $classes) . '">';
        $after_link = '</span>';
    }

    $attrs['img'] = json_decode($attrs['img'], true);

    $html = $before_link .'<img class="dragifywp-logo__image" src="'.$attrs['img']['url'].'"/>'. $after_link;
