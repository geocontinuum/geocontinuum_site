            ...
            "content" => [
                array(
                    ...
                    "responsive_styles" => array(
                        "@id .dragifywp-advanceText__content" => array(
                            "fontSize" => "{{@model}}px"
                        )
                    )
                ),
                ...
                array(
                    ...
                    "directives" => [
                        array(
                            ...
                            "attrs" => array(
                                "selector" => "@id .dragifywp-advanceText__content"
                            )
                        )
                    ]
                ),
...
<div class="dragifywp-dnd__emptyItem" style="height:60px;" ng-if="!$ctrl.edited.content">
    <span class="dragifywp-dnd__emptyItem__label">Advanced Text</span>
</div>

<div class="dragifywp-advanceText {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.hidden]">
    <dragifywp-tag-wrap id="{{$ctrl.edited.attrs.id}}" tag-class="$ctrl.edited.attrs.class" model="$ctrl.edited" tag-name="$ctrl.edited.attrs.tag" selector="dragifywp-advanceText__content"></dragifywp-tag-wrap>
</div>
<?php
...
add_shortcode('dragifywp_advance_text', 'dragifywp_advance_text_callback');
function dragifywp_advance_text_callback($attrs, $content = '') {
	...
	ob_start(); ?>
	<div class="dragifywp-advanceText <?php echo implode(' ', $classes); ?>"<?php echo $animation; ?>>
		<<?php echo $attrs['tag']; ?> class="dragifywp-advanceText__content<?php echo $attrs['class']; ?>"<?php echo $id; ?>><?php echo do_shortcode($content); ?></<?php echo $attrs['tag'] ?>>
	</div>
	<?php
	return ob_get_clean();
}