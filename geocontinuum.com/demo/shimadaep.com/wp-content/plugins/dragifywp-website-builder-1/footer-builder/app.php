...
require_once('DragifyWPFooterBuilderAPI.php');
require_once "DragifyWPFooterBuilderAdmin.php";
require_once "DragifyWPFooterBuilderClient.php";

function dragifywp_get_footer_options() {
	$options = get_option('dragifywpFooterOptions');

	return is_array($options) ? $options : array();
}

function dragifywp_get_footer_setting($id) {
	$footer_setting = get_post_meta($id, 'dragifywp-page-footer-setting', true);
	return !empty($footer_setting) && is_array($footer_setting) ? $footer_setting : array();
}

function dragifywp_get_footer_id($data) {
	...
	if (!empty($footer_id)) {
		...
		if (!$footer || $footer->post_status != 'publish' || $footer->post_type != 'dragifywp_footer') {
			$footer_id = 0;
		}
	}
...
function dragifywp_fb_get_post_types() {
	return get_post_types(array('show_ui' => true), 'objects');
}

function dragifywp_get_footer_data() {
	$data = array();
	if (is_home()) {
		$data['type'] = 'home';
	} else if (is_search()) {
...
function dragifywp_footer_builder_render(){
	DragifyWPFooterBuilderClient::render();
}
