            ...
            "content" => [
                ...
                array(
                    ...
                    "styles" => array(
                        ...
                        "@id + .dragifywp-aside__backdrop" => array(
                            "zIndex" => "{{@model - 1}}"
                        )
                    )
                )
        ...
        array(
            ...
            "content" => [
                ...
                array(
                    ...
                    "styles" => array(
                        "@id.dragifywp-aside--background-image" => array(
                ...
                array(
                    ...
                    "styles" => array(
                        "@id.dragifywp-aside--background-gradient" => array(
...
ob_start();
?>
<div class="dragifywp-aside dragifywp-hb-menu {{::$ctrl.service.getID($ctrl.nav)}}"
     ...
     ng-class="[$ctrl.nav.attrs.hidden, $ctrl.nav.attrs.class,
                        'dragifywp-aside--background-' + $ctrl.nav.attrs.bg_style,
                        'dragifywp-aside--flex-' + $ctrl.nav.attrs.layout,
                        {'dragifywp-aside--controlled-by-burger' : $ctrl.nav.attrs.controlled_by_burger,
                        'dragifywp-aside--right':$ctrl.nav.attrs.right,
                        'dragifywp-aside--scroll': $ctrl.nav.attrs.scroll,
                        'dragifywp-dnd__editedNav' : $ctrl.nav.kept}]">
    <div class="dragifywp-dnd__visual dragifywp-dnd__visual--nav dragifywp-dnd__visual--bold lvh__{{::$ctrl.service.getID($ctrl.nav)}}" ng-click="$ctrl.edit()" dragifywp-detect-hover-node-vertical="$ctrl.nav"></div>

    <div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--vNav" ng-if="!$ctrl.nav.content.length">
        <span class="dragifywp-dnd__emptyItem__label">Vertical Nav</span>
    </div>

    <vertical-section-list nav="$ctrl.nav"></vertical-section-list>
</div>

<div class="dragifywp-aside__backdrop"></div>
<?php
...
function lhb_vertical_nav_callback($attrs, $content = null) {
    ...
    if (dragifywp_to_boolean($attrs['controlled_by_burger'])) {
        $classes[] = 'dragifywp-aside--controlled-by-burger';
        ...
        $fixed_html .= '<div class="dragifywp-aside__backdrop"></div>';
    }

    if($attrs['bg_style'] != 'none'){
        $classes[] = 'dragifywp-aside--background-'.$attrs['bg_style'];
    }

    if (dragifywp_to_boolean($attrs['right'])) {
        $classes[] = 'dragifywp-aside--right';
    }

    if(dragifywp_to_boolean($attrs['scroll'])){
        $classes[] = 'dragifywp-aside--scroll';
    }

    $classes[] = $attrs['class_id'];
    ob_start(); ?>
    <div class="dragifywp-aside <?php echo implode(' ', $classes); ?>"<?php echo $datas; ?> <?php echo $el_id; ?>>
        <div class="dragifywp-aside__container dragifywp-aside__container--flex-<?php echo $attrs['layout']; ?>"><?php echo do_shortcode($content); ?></div>
    </div>
    <?php echo $fixed_html; ?>
    <?php
    $html .= ob_get_clean();
