...
ob_start();
?>
<div class="dragifywp-dnd__emptyItem dragifywp-dnd__emptyItem--h" style="width:140px;" ng-if="!$ctrl.content.length">
    <span class="dragifywp-dnd__emptyItem__label">List</span>
</div>

<ul class="dragifywp-header__list {{::$ctrl.getID()}}"
    ng-class="[{'dragifywp-header__list--hideLeft':$ctrl.edited.attrs.hide_left, 'dragifywp-header__list--hideRight':$ctrl.edited.attrs.hide_right}, $ctrl.edited.attrs.class, $ctrl.edited.attrs.hidden]">
    <li class="dragifywp-header__list__item" ng-repeat="item in $ctrl.model.content">
...
function lhb_h_list_callback($attrs, $content = null) {
    ...
    if (dragifywp_to_boolean($attrs['hide_left'])) {
        $classes[] = 'dragifywp-header__list--hideLeft';
    }

    if (dragifywp_to_boolean($attrs['hide_right'])) {
        $classes[] = 'dragifywp-header__list--hideRight';
    }
    ...
    $html = '<ul class="dragifywp-header__list ' . implode(' ', $classes) .'">' . do_shortcode($content) . '</ul>';
...
function lhb_h_list_item_callback($attrs) {
    ...
    $html = '<li class="dragifywp-header__list__item '.$attrs['class_id'].'">';
