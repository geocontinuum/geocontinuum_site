...
class DragifyWPMenuItemDefault extends DragifyWPMenuItem {
	...
	function get_start_el(){
		...
		if ($this->depth == 0) {
			$this->add_class('dragifywp-menu-item-top');
		}

		if ($this->parent_type_label != '[Column]') {
			$this->add_class('dragifywp-menu-column');
		}

		if ($this->item->has_row) {
			$this->add_class('dragifywp-menu-mega-menu');
		}
				...
				switch ($this->args->current_menu_type) {
					case 'post':
						if ($item_type == 'post_type' && $object_id == $current_object_id) {
							$this->add_class('dragifywp-menu-current-menu-item');
						}
